...
#
# Borrowed from here: https://github.com/dpallot/simple-websocket-server
# Modified by Roman V.M.
"""
...
class WebSocket(object):
    ...
    def __init__(self, server, sock, address, request=None):
        self.server = server
        self.client = sock
        ...
        if self.opcode == CLOSE:
            ...
            if self.opcode != STREAM:
                if self.opcode == PING or self.opcode == PONG:
                    raise WebSocketError('control messages can not be fragmented')
            ...
            if self.request is None:
                ...
                if not data:
                    ...
                    if b'\r\n\r\n' in self.headerbuffer:
                        ...
                        raise WebSocketError('HTTP header terminator not found!')
            # handshake rfc 6455
            try:
                ...
                raise WebSocketError('handshake failed: %s', str(e))
        ...
        try:
            while self.sendq:
                ...
                if remaining is not None:
                    ...
                    if opcode == CLOSE:
                        raise WebSocketError('received client close')
        except Exception:
            self.handle_error()
    ...
    def close(self, status=1000, reason=u''):
        if self.handshaked:
            try:
                ...
                pass
        WebSocket.close(self, status, reason)
